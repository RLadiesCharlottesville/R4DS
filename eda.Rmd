...
# Overlaying:
diamonds %>%
    gather(key = "key", value = "value", x:z) %>% # gether into short form for ggplot
    ggplot(mapping = aes(x = value, color = key)) + 
...
**Answer**: there are 23 diamonds that are .99 carats, and 1,558 diamonds that are 1 carat. I assume that dealers probably just rounded up. 
...
**Answer**: `xlim()` and `ylim()` change the axis limits and drop data observations that don't fall within the limit (so it won't keep half a bar) while `coord_cartesian()` keeps all observations. <span style = "color:blue" Why do the two plots still look different though?</span>
...
# Plot bar chart of cut (discrete variable):
diamonds %>%
    mutate(cut = ifelse(cut == "Fair", NA, cut)) %>%
    ggplot(aes(x = cut)) +
    geom_bar()

# Note warning message saying 1610 variables couldn't be counted 
```
